#!/user/bin/env python
# yangyi@2020-2022
# doa estimation via self-attention
# parameters:282k  computation:5.2G/10s  2.1G/4s
...
class proposed(nn.Module):
    def __init__(self, fftsize=512, window_size=400, stride=100, channel=4, attention_type='TF', causal=False):
        ...
        self.stft = STFT(fftsize=fftsize, window_size=window_size, stride=stride, trainable=True)
        # increase or decrease the channel dim
    ...
    def forward(self, x):    # b n c
        ...
        for i in range (4):
            ...
            if (self.attention_type=='TF' or self.attention_type=='TFC'):
                x_in3 = x_out2                    # (B,8,T,64)
                x_out3, plot_weight_f = self.shared_block[1+i*3](x_in3,causal=self.causal)   # (B,8,T,64)
            else:
                x_out3 = x_out2
                
            # step5:self-attention C
            if (self.attention_type=='TFC'):
                x_in4 = (x_out2+x_out3).permute(0,3,2,1)   # (B,4,T,8)
                x_out4, plot_weight_c = self.shared_block[2+i*3](x_in4,causal=self.causal)   # (B,64,T,8)
                x_out4 = x_out4.permute(0,3,2,1)
            else:
                x_out4 = x_out3

            x_in = x_out3+x_out4

        x_out = x_in.squeeze(-1).permute(0,2,1) # b t 8
...
if __name__ == "__main__":
    ...
    model = proposed(attention_type='TF', causal=False)
